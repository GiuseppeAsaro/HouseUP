...
@router.get(
    ...
    summary="List my feedback snapshots",
    description="Read feedback snapshots from authenticated user profile only.",
)
async def list_my_feedbacks(
    current_user: User = Depends(get_current_user),
):
    try:
        return feedbacks_service.list_feedbacks_from_user_list(current_user)
    except PermissionError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc))
    except RuntimeError as exc:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(exc))
